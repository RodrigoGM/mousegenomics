...
class BAM2VCF extends QScript {
  ...
  def script() {
    ...
    val hc2 = new HaplotypeCaller with CommonArguments
    ...
    hc.pcr_indel_model = org.broadinstitute.gatk.tools.walkers.haplotypecaller.PairHMMLikelihoodCalculationEngine.PCR_ERROR_MODEL.NONE
    hc.out = swapExt(applyRecalibration.out, ".dd.ir.bqsr.bam", ".g.vcf.gz")
    ...
    hc2.scatterCount = qscript.scatter
    hc2.input_file +:= applyRecalibration.out
    hc2.nct = qscript.nct
    hc2.pairHMM = org.broadinstitute.gatk.utils.pairhmm.PairHMM.HMM_IMPLEMENTATION.VECTOR_LOGLESS_CACHING
    hc2.pcr_indel_model = org.broadinstitute.gatk.tools.walkers.haplotypecaller.PairHMMLikelihoodCalculationEngine.PCR_ERROR_MODEL.NONE
    hc2.out = swapExt(applyRecalibration.out, ".dd.ir.bqsr.bam", ".vcf.gz")
    hc2.variant_index_type = org.broadinstitute.gatk.utils.variant.GATKVCFIndexType.LINEAR
    hc2.variant_index_parameter = 128000 

    add(targetCreator, indelRealigner, bqsr, bqsr2, analyzeCovariates, applyRecalibration, hc, hc2)
